...
export function generateTeam(allowedTypes, maxLevel, characterCount, field) {
  ...
  function getRandomCell() {
    ...
    if (allowedTypes.includes(Bowman) || allowedTypes.includes(Swordsman) || allowedTypes.includes(Magician)) {
    ...
    if (allowedTypes.includes(Daemon) || allowedTypes.includes(Undead) || allowedTypes.includes(Vampire)) {
      for (let i = 0; i < field.length; i++) {
        if ((i + 1) % 8 === 0 || (i + 2) % 8 === 0) {
          playersCellsIndexes.push(i);
        }
